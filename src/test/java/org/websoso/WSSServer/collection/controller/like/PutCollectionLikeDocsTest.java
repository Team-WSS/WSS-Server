...
/**
 * {@code PUT /collections/{collectionId}/likes}의 실제 응답을 REST Docs로 문서화하는 테스트.
 * 성공 응답과 이 호출 경로가 정의한 실패 응답을 요청으로 재현해 확인한 그대로 명세에 남긴다.
 *
 * <p>멱등성 자체는 Application 테스트가 확인한다. 여기서는 반복 호출이 클라이언트에게 같은 성공 응답으로
 * 보인다는 사실과 각 정책 위반이 어떤 상태 코드와 본문으로 나가는지를 문서로 남긴다.
 */
@AutoConfigureRestDocs
@AuthenticatedControllerTest(CollectionLikeController.class)
class PutCollectionLikeDocsTest {
    ...
    private static final String SUMMARY = "컬렉션 좋아요 등록";

    private static final String DESCRIPTION = String.join("\n",
            "Access Token으로 인증한 사용자가 컬렉션에 좋아요를 등록합니다.",
            "자신이 만든 컬렉션에도 좋아요를 누를 수 있습니다.",
            "",
            "같은 요청을 반복해도 좋아요는 하나만 남고 응답도 같습니다.",
            "이미 좋아요한 컬렉션에 다시 요청해도 오류가 아니라 204를 반환하므로,",
            "클라이언트는 현재 좋아요 상태를 몰라도 \"좋아요 상태로 만든다\"는 의도만 보내면 됩니다.",
            "",
            "볼 수 없는 컬렉션에는 좋아요를 누를 수 없습니다.",
            "다른 사용자의 비공개 컬렉션과 차단 관계 사용자의 컬렉션이 여기에 해당합니다.",
            "",
            ...
            errorLine(PRIVATE_COLLECTION_ACCESS) + " (다른 사용자의 비공개 컬렉션에는 좋아요를 누를 수 없습니다.)",
            errorLine(BLOCKED_USER_ACCESS) + " (어느 방향이든 차단 관계면 좋아요를 누를 수 없습니다.)",
    ...
    @DisplayName("컬렉션 좋아요 등록 성공(204, 본문 없음) 응답을 문서화한다")
    @Test
    void documentLikeSuccess() throws Exception {
        mockMvc.perform(likeRequest().with(accessToken(USER_ID)))
                .andExpect(status().isNoContent())
                .andExpect(content().string(""))
                .andDo(document("collection-likes-put",
                        resource(collection().build())));
    }

    @DisplayName("이미 좋아요한 컬렉션에 다시 요청해도 같은 성공 응답을 준다")
    @Test
    void repeatedLikeReturnsSameResponse() throws Exception {
        mockMvc.perform(likeRequest().with(accessToken(USER_ID)))
                .andExpect(status().isNoContent());
    ...
    @Test
    void documentLikeWithExpiredAccessToken() throws Exception {
        mockMvc.perform(likeRequest().with(expiredAccessToken(USER_ID)))
                ...
                .andDo(document("collection-likes-put-access-token-expired",
                        resource(collectionError().build())));
    }

    @DisplayName("Authorization 헤더가 없는 요청의 401 AUTH-001 응답을 문서화한다")
    @Test
    void documentLikeWithInvalidToken() throws Exception {
        mockMvc.perform(likeRequest())
                .andExpect(status().isUnauthorized())
                .andExpect(content().contentTypeCompatibleWith(APPLICATION_JSON))
                .andExpect(jsonPath("$.code").value(INVALID_TOKEN.getCode()))
                .andExpect(jsonPath("$.message").value(INVALID_TOKEN.getDescription()))
                .andDo(document("collection-likes-put-invalid-token",
                        resource(collectionError().build())));
    }

    @DisplayName("Refresh Token으로 인증한 요청의 401 AUTH-003 응답을 문서화한다")
    @Test
    void documentLikeWithWrongTokenType() throws Exception {
        mockMvc.perform(likeRequest().with(refreshToken(USER_ID)))
                .andExpect(status().isUnauthorized())
                .andExpect(content().contentTypeCompatibleWith(APPLICATION_JSON))
                .andExpect(jsonPath("$.code").value(WRONG_TOKEN_TYPE.getCode()))
                .andExpect(jsonPath("$.message").value(WRONG_TOKEN_TYPE.getDescription()))
                .andDo(document("collection-likes-put-wrong-token-type",
                        resource(collectionError().build())));
    }

    @DisplayName("다른 사용자의 비공개 컬렉션 요청의 403 COLLECTION-006 응답을 문서화한다")
    @Test
    void documentLikeOnPrivateCollection() throws Exception {
        givenLikeThrows(new CustomCollectionException(
                PRIVATE_COLLECTION_ACCESS, "only the owner of the collection can access a private collection"));

        documentLikeError(PRIVATE_COLLECTION_ACCESS, "collection-likes-put-private-collection");
    }

    @DisplayName("차단 관계인 사용자의 컬렉션 요청의 403 BLOCK-007 응답을 문서화한다")
    @Test
    void documentLikeOnBlockedUserCollection() throws Exception {
        givenLikeThrows(new CustomBlockException(
                BLOCKED_USER_ACCESS, "cannot access content because either user has blocked the other"));

        documentLikeError(BLOCKED_USER_ACCESS, "collection-likes-put-blocked-user");
    }

    @DisplayName("존재하지 않는 컬렉션 요청의 404 COLLECTION-001 응답을 문서화한다")
    @Test
    void documentLikeOnUnknownCollection() throws Exception {
        givenLikeThrows(new CustomCollectionException(
                COLLECTION_NOT_FOUND, "collection with the given id is not found"));

        documentLikeError(COLLECTION_NOT_FOUND, "collection-likes-put-collection-not-found");
    }

    @DisplayName("토큰은 유효하지만 사용자가 없는 요청의 404 USER-006 응답을 문서화한다")
    @Test
    void documentLikeWithUnknownUser() throws Exception {
        ...
        documentLikeError(USER_NOT_FOUND, "collection-likes-put-user-not-found");
    }

    private void givenLikeThrows(RuntimeException exception) {
        willThrow(exception).given(collectionLikeApplication).create(any(User.class), eq(COLLECTION_ID));
    }
     ...
     */
    private void documentLikeError(ICustomError error, String documentIdentifier) throws Exception {
        mockMvc.perform(likeRequest().with(accessToken(USER_ID)))
    ...
    private ResourceSnippetParametersBuilder collection() {
        return ResourceSnippetParameters.builder()
                ...
                .pathParameters(parameterWithName("collectionId")
                        ...
                        .description("좋아요할 컬렉션 ID"));
    }

    private MockHttpServletRequestBuilder likeRequest() {
