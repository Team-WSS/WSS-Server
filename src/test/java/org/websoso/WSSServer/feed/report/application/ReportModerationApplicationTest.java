...
class ReportModerationApplicationTest {
    ...
    void marksFeedAsSpoilerAfterCommit() {
        givenFeedModeration(ReportedType.SPOILER, 3, ReportModerationAction.MARKED_AS_SPOILER, "spoiler message");
        given(feedService.markSpoilerIfNotMarked(FEED_ID)).willReturn(true);

        reportModerationApplication.moderateFeed(REPORTER_ID, FEED_ID, ReportedType.SPOILER);
    ...
    void hidesFeedAfterCommit() {
        givenFeedModeration(ReportedType.IMPERTINENCE, 3, ReportModerationAction.HIDDEN, "hidden message");
        given(feedService.hideIfNotHidden(FEED_ID)).willReturn(true);

        reportModerationApplication.moderateFeed(REPORTER_ID, FEED_ID, ReportedType.IMPERTINENCE);
    ...
    void marksCommentAsSpoilerAfterCommit() {
        givenCommentModeration(ReportedType.SPOILER, 3, ReportModerationAction.MARKED_AS_SPOILER, "comment message");
        given(commentService.markSpoilerIfNotMarked(COMMENT_ID)).willReturn(true);

        reportModerationApplication.moderateComment(REPORTER_ID, COMMENT_ID, ReportedType.SPOILER);
    ...
    void doesNotModerateBelowLimit() {
        givenFeedModeration(ReportedType.SPOILER, 2, ReportModerationAction.NONE, "not moderated message");
    ...
    private void givenFeedModeration(ReportedType reportedType, int reportedCount,
        ...
        given(reportService.countByFeedAndReportedType(feed, reportedType)).willReturn(reportedCount);
    ...
    private void givenCommentModeration(ReportedType reportedType, int reportedCount,
                                        ReportModerationAction action, String message) {
        ...
        given(reportService.countByCommentAndReportedType(comment, reportedType)).willReturn(reportedCount);
