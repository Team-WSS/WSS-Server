...
- 컬렉션 카드에는 대표 작품과 최근 추가된 작품 최대 5개, 포함 작품 수를 함께 준다.
  대표 작품이 최근 추가 작품에 포함될 수 있으므로 둘을 합치지 않고 따로 내보낸다.
...
- 포함 작품은 식별자만 주지 않고 화면에 필요한 정보(제목, 작가, 표지, 완결 여부, 평점, 평점 수)를 함께 준다.
...
### 12.3 유스케이스 트랜잭션 경계는 Application이 소유하고 Service는 메서드 단위 속성을 명시한다
